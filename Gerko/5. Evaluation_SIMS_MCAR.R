...
load("Workspaces/4. Evaluations_CCA_MCAR.RData")
...
pmm1 <- sim1.pmm %>% Reduce("+", .) / 100 # average over the 100 sims
pmm2 <- sim2.pmm %>% Reduce("+", .) / 100 # average over the 100 sims
pmm3 <- sim3.pmm %>% Reduce("+", .) / 100 # average over the 100 sims

# impute and exclude NA's as imputations
pmm.exclude1 <- sim1.pmm.exclude %>% Reduce("+", .) / 100 # average over the 100 sims
pmm.exclude2 <- sim2.pmm.exclude %>% Reduce("+", .) / 100 # average over the 100 sims
pmm.exclude3 <- sim3.pmm.exclude %>% Reduce("+", .) / 100 # average over the 100 sims

# impute and set all NA's to missing and overimpute
pmm.overimpute1 <- sim1.pmm.overimpute %>% Reduce("+", .) / 100 # average over the 100 sims
pmm.overimpute2 <- sim2.pmm.overimpute %>% Reduce("+", .) / 100 # average over the 100 sims
pmm.overimpute3 <- sim3.pmm.overimpute %>% Reduce("+", .) / 100 # average over the 100 sims

# remove all rows with NA's before imputation (lower sample size)
pmm.removebefore1 <- sim1.removebefore %>% Reduce("+", .) / 100 # average over the 100 sims
pmm.removebefore2 <- sim2.removebefore %>% Reduce("+", .) / 100 # average over the 100 sims
pmm.removebefore3 <- sim3.removebefore %>% Reduce("+", .) / 100 # average over the 100 sims
...
save(pmm1, pmm2, pmm3, 
     ...
     file = "Workspaces/5. Evaluation_SIMS_MCAR.RData")
